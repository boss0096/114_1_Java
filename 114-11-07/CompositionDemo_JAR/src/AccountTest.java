...
//        printCustomerAccounts(customers);
    ...
    public static void operation(List<Account> customers) {
        ...
        Account selectedAccount; // 移除冗餘的 null 初始值
        while (true) {
            menu();
            ...
            switch (choice) {
                case 1:
                    ...
                    System.out.print("輸入身分證號: ");
                    String ownerID = scanner.nextLine();
                    System.out.print("輸入初始餘額: ");
                    double initialBalance = scanner.nextDouble();
                    scanner.nextLine(); // 清除輸入後的換行符號
                ...
                case 4:
                    ...
                    break;
                case 5: // 存款
                    System.out.print("輸入欲存款的帳戶號碼: ");
                    String depAcc = scanner.nextLine();
                    selectedAccount = customerInAction(customers, depAcc);
                    ...
                    try {
                        ...
                        System.out.println("存款成功，最新餘額: " + String.format("%.2f", selectedAccount.getBalance()));
                    } catch (IllegalArgumentException e) {
                        System.out.println("存款失敗: " + e.getMessage());
                    }
                    break;
                case 6: // 提款
                    System.out.print("輸入欲提款的帳戶號碼: ");
                    String witAcc = scanner.nextLine();
                    selectedAccount = customerInAction(customers, witAcc);
                    if (selectedAccount == null) break;
                    ...
                    try {
                        ...
                        System.out.println("提款成功，最新餘額: " + String.format("%.2f", selectedAccount.getBalance()));
                    } catch (IllegalArgumentException e) {
    ...
    public static void deleteCustomer(List<Account> customers, String accountNumber) {
        for (int i = 0; i < customers.size(); i++) {
            if (customers.get(i).getAccountNumber().equals(accountNumber)) {
                ...
                System.out.println("刪除客戶成功: " + removedAccount.getAccountNumber() + " (" + removedAccount.getOwner().getName() + ")");
                return;
            }
        }
        System.out.println("找不到指定的帳戶號碼: " + accountNumber);
    ...
    public static void printCustomerInfo(Account account) {
        ...
        System.out.println(account); // 不需要明確呼叫 toString()
    }
